...
int main(){
       ...
       int waypoints [20][2];
        ...
        while ( fgets(line, sizeof(line), file) ){ //read each line until end of f$

                char* split=strtok (line," "); //split line between spaces ""
                waypoints[index][0]=atol(split); //convert first part to int, stor$
                split = strtok (NULL, " "); // get next part after space
                waypoints[index][1]=atol(split); //convert second part to int, sto$
	...
	int local_y = 0;
        ...
        while(a < 4){
                ...
                int angle = atan2(local_y , local_x)/M_PI * 180;        

                printf( "new local x value is %i new loacl y value is %i , angle value is %i" , local_x , local_y, angle  ) ;
